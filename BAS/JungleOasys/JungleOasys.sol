...
contract JungleOasys is ERC1155, AccessControl{
    ...
    function addServer(address account) public onlyAdmin {
    ...
    function addMarketer(address account) public  onlyAdmin {
        require(hasRole(SERVER_ROLE, account) == false);
        require(hasRole(ASSET_ADMIN_ROLE, account) == false);
        grantRole(MARKETER_ROLE, account);
    }
    
    function addAssetAdmin(address account) public  onlyAdmin {
        require(hasRole(SERVER_ROLE, account) == false);
        require(hasRole(MARKETER_ROLE, account) == false);
        grantRole(ASSET_ADMIN_ROLE, account);
    }

    function createFungible() public {

    }

    function createSemiFungible public onlyRole(MARKETER_ROLE) {
